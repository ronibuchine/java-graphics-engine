...
public class GeometriesTests {
    ...
    public void testFindIntersections() {
        ...
        Geometries geoms2 = new Geometries(
                new Polygon(new Point3D(0, 20, 0), new Point3D(5, 20, 5), new Point3D(5, 20, 10),
                        new Point3D(0, 20, 20), new Point3D(-5, 20, 5)),
        ...
        assertEquals("Ray intersects all geometries",
                List.of(new Point3D(0, 20, 10), new Point3D(0, 7, 10), new Point3D(0, 13, 10), new Point3D(0, 25, 10),
                        new Point3D(0, 33, 10)),
                geoms2.findIntersections(new Ray(new Point3D(0, 0, 10), new Vector(0, 1, 0))));

        // ============== Equivalence Partitions Tests ==============
        // some shapes but not all intersects (EP)
        Geometries allGeoms = new Geometries(geoms1, geoms2);
        // One of the Polygons is in both Geometries collections so it's intersection
        // point will show up twice
        assertEquals("Ray intersects all geometries",
