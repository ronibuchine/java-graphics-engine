...
public class SceneBuilder {
    ...
    public void loadAmbientLight() {
        if (sceneDesc.ambientLightAttributes.get("color") == null)
            return;
    ...
    public void loadLightSources() {
        ...
        for (Map<String, String> light : sceneDesc.lights) {
            ...
            if (light.get("kc") != null)
                kC = Double.parseDouble(light.get("kc"));
            if (light.get("kl") != null)
                kL = Double.parseDouble(light.get("kl"));
            if (light.get("kq") != null)
                kQ = Double.parseDouble(light.get("kq"));
            if (light.get("beam") != null)
                beam = Double.parseDouble(light.get("beam"));
            ...
            switch (light.get("type")) {
                ...
                case "spot":
                    ...
                    direction = new Vector(parsePoint(light.get("direction")));
                    scene.lights
                            .add(new SpotLight(color, point, direction).setBeam(beam).setKc(kC).setKl(kL).setKq(kQ));
            }
        }
    }
    ...
    public void loadGeometries() {
        ...
        for (Map<String, String> cylinder : sceneDesc.cylinders) {
            ...
            geometries.add(new Cylinder(radius, new Ray(point, direction), height).setEmission(color)
                    .setMaterial(parseMaterial(cylinder)));
        }
        ...
        for (Map<String, String> plane : sceneDesc.planes) {
            ...
            if (plane.containsKey("normal")) {
                ...
                geometries.add(new Plane(normal, point).setEmission(color).setEmission(color)
                        .setMaterial(parseMaterial(plane)));
            } else {
                Point3D p0 = parsePoint(plane.get("p0"));
                Point3D p1 = parsePoint(plane.get("p1"));
                Point3D p2 = parsePoint(plane.get("p2"));
                geometries.add(
                        new Plane(p0, p1, p2).setEmission(color).setEmission(color).setMaterial(parseMaterial(plane)));
            }
        }
        ...
        for (Map<String, String> polygon : sceneDesc.polygons) {
            ...
            for (Map.Entry<String, String> attr : polygon.entrySet()) {
                if (attr.getKey().startsWith("p"))
                    list.add(parsePoint(attr.getValue()));
            }
            Color color = parseColor(polygon.get("color"));
            geometries.add(new Polygon(list.toArray(new Point3D[list.size()])).setEmission(color).setEmission(color)
                    .setMaterial(parseMaterial(polygon)));
        }
        ...
        for (Map<String, String> sphere : sceneDesc.spheres) {
            ...
            geometries.add(new Sphere(center, radius).setEmission(color).setEmission(color)
                    .setMaterial(parseMaterial(sphere)));
        }
        ...
        for (Map<String, String> tube : sceneDesc.tubes) {
            ...
            geometries.add(new Tube(radius, new Ray(point, direction)).setEmission(color).setEmission(color)
                    .setMaterial(parseMaterial(tube)));
        }
    ...
    private Material parseMaterial(Map<String, String> geo) {
        ...
        if (geo.get("ks") != null)
            kS = Double.parseDouble(geo.get("ks"));
        if (geo.get("shininess") != null)
            shininess = Integer.parseInt(geo.get("shininess"));
        if (geo.get("kr") != null)
